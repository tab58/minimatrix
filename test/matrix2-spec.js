...
describe('Matrix2', () => {
  ...
  describe('Arithmetic Operations', () => {
    ...
    it('should compute the outer product of 2 vectors', () => {
      const c = new Matrix2();
      c.set(3, 5, 7, 11);
      const a = new Vector2(13, 17);
      const b = new Vector2(19, 23);
      c.getOuterProduct(a, b);
      expect(c.elements[0]).to.be.eql(13 * 19);
      expect(c.elements[1]).to.be.eql(17 * 19);
      expect(c.elements[2]).to.be.eql(13 * 23);
      expect(c.elements[3]).to.be.eql(17 * 23);
      const s = 3;
      const d = new Matrix2();
      d.getOuterProduct(a, b, s);
      expect(d).to.be.eql(c.clone().multiplyScalar(s));
    });
    it('should add to itself the outer product of 2 vectors', () => {
      const c = new Matrix2();
      c.set(3, 5, 7, 11);
      const a = new Vector2(13, 17);
      const b = new Vector2(19, 23);
      c.addOuterProduct(a, b);
      expect(c.elements[0]).to.be.eql(13 * 19 + 3);
      expect(c.elements[1]).to.be.eql(17 * 19 + 7);
